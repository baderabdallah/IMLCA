...
class KPIsTrackerNode:
    ...
    def _update_overall_avg_velocity(self, msg):
        ...
        self.overall_avg_of_velocity_samples = (
            self.overall_sum_of_velocity_samples
            / self.overall_count_of_velocity_samples
        )

    def _update_lane_changes(self, msg):
        # (possibly) update the count for lane changes
        if (
            self.current_lane is not None
            and self.current_lane != msg.ego_info.lane_number
        ):
            self.lane_changes += 1
