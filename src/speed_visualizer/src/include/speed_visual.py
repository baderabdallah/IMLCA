...
class SpeedVisual:
    ...
    def __init__(self):
        self._fig, self._ax = plt.subplots(
            1, 1, figsize=(FIGURE_WIDTH, FIGURE_HEIGHT), subplot_kw={'projection': 'polar'}
        )
    ...
    def draw_speedometer_visual(self, current_speed):
        ...
        for angle in np.linspace(0, np.pi, 10):
            ...
            self._ax.text(angle, 11, f'{int(MAX_SPEED * angle / np.pi)}', ha='center', va='center')
        ...
        # Create text visual
        self._ax.text(-np.pi/2, 3, f'{current_speed} km/h', ha='center', va='center', fontsize=16, style='italic')
